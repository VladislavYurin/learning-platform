...
import ru.mentor.grpc.MentorTagsGrpcClient;
...
/**
 * Редирект сервис для управления тэгами ментора.
 */
@Service
@RequiredArgsConstructor
@Slf4j
public class RedirectMentorTagServiceImpl implements RedirectMentorTagService {

    private final MentorTagsGrpcClient grpcClient;
    ...
    public List<MentorTagDto> allMentorTags() {
        ...
        try {
            AllMentorTagsRequset requset = AllMentorTagsRequset.newBuilder()
                                                               .setHeader(header)
                                                               .build();

            AllMentorTagsResponse response = grpcClient.getAllMentorTags(requset);
    ...
    public MentorTagDto createCustomMentorTag(MentorTagDtoCreateRequest createMentorTagRequset) {
        ...
        try {
            ...
            MentorTagResponse response = grpcClient.createCustomMentorTag(request);
    ...
    @Override
    public List<MentorTagDto> getCurrentMentorTags(Long mentorId) {
        String requestId = Optional.ofNullable(MDC.get(MdcKeys.REQUEST_ID)).orElse("");
        Header header = headerFactory.create(requestId);
        Long userId = resolveUserId();

        log.debug(
                "[userId={}] Получен запрос на получение тэгов конкретного ментора [ID={}].",
                userId,
                mentorId
        );

        try {
            GetCurrentMentorTagsRequest request = mapper.toCurrentMentorTagsRequest(header, mentorId);
            MentorTagsResponse response = grpcClient.getCurrentMentorTagsRequest(request);

            log.debug(
                    "[userId={}] Успешно получен ответ от mentor-service на получение тэгов конкретного ментора [ID={}]",
                    userId,
                    mentorId
            );

            return mapper.toListMentorTagDto(response);
        } catch (Exception e) {
            log.error(
                    "[userId={}] Ошибка при вызове mentor-service во время получения тэгов конкретного ментора.",
                    userId,
                    e
            );
            throw e;
    ...
    public MentorTagAttachResponseDto attachMentorTags(MentorTagsAttachRequestDto attachRequest) {
        ...
        checkUserRights(attachRequest.getMentorId(), requestId);
        ...
        try {
            ...
            AttachMentorTagsResponse response = grpcClient.attachMentorTagsRequest(request);
    ...
    public MentorTagDetachResponseDto detachMentorTag(MentorTagDetachRequestDto detachRequest) {
        ...
        checkUserRights(detachRequest.getMentorId(), requestId);

        try {
            DetachMentorTagRequest request = mapper.toDetachMentorTagRequest(detachRequest, header);
            DetachMentorTagResponse response = grpcClient.detachMentorTagResponse(request);
    ...
    private void checkUserRights(Long userId, String requestId) {
        if (!userId.equals(userService.getCurrentUserId())) {
            throw new CustomAccessDeniedException("Пользователь не является ментором", requestId);
        }
