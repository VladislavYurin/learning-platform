...
import ru.mentor.admin.AdminMentorServiceGrpc.AdminMentorServiceBlockingStub;
...
@AllArgsConstructor
@Slf4j
public class AdminMentorTagsGrpcClient {
    /**
     * Блокирующий gRPC-stub Mentor Service.
     */
    @GrpcClient("admin-mentor-client")
    private AdminMentorServiceBlockingStub blockingStub;
    ...
    public MentorTagResponse createCustomMentorTag(
        ...
        try {
            ...
            throw new GrpcRetryException(
                    String.format(
                            "Ошибка отправки gRPC запроса в admin-mentor-service при создании тэга. cause=%s",
                            e.getMessage()
    ...
    public AllMentorTagsResponse getAllMentorTags(
        ...
        try {
            ...
            throw new GrpcRetryException(
                    String.format(
                            "Ошибка отправки gRPC запроса в admin-mentor-service при получении всех тэгов. cause=%s",
                            e.getMessage()
                    ), request.getHeader().getRequestId()
            );
        }
    ...
    public AttachMentorTagsResponse attachMentorTagsRequest(
        ...
        try {
            ...
            throw new GrpcRetryException(
                    String.format(
                            "Ошибка отправки gRPC запроса в admin-mentor-service при привязке тэга. cause=%s",
                            e.getMessage()
                    ), request.getHeader().getRequestId()
            );
        }
    ...
    public DetachMentorTagResponse detachMentorTagResponse(
        ...
        try {
            ...
            throw new GrpcRetryException(
                    String.format(
                            "Ошибка отправки gRPC запроса в admin-mentor-service при отвязке тэга. cause=%s",
                            e.getMessage()
                    ), request.getHeader().getRequestId()
            );
        }
