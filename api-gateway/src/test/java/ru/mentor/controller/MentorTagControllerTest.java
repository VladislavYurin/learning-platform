...
@WebMvcTest(MentorTagController.class)
@AutoConfigureMockMvc(addFilters = false)
public class MentorTagControllerTest {
    @Autowired
    ...
    void getAllMentorTags_success() {
        Mockito.when(mentorTagService.allMentorTags())
    ...
    void createCustomMentorTag_success() {
        ...
        Mockito.when(mentorTagService.createCustomMentorTag(Mockito.any(MentorTagDtoCreateRequest.class)))
               .thenReturn(constructMentorTagDto());

        mockMvc.perform(MockMvcRequestBuilders.post("/mentor-tag/add")
    ...
    void attachMentorTags_success() {
        ...
        Mockito.when(mentorTagService.attachMentorTags(Mockito.any(MentorTagsAttachRequestDto.class)))
               .thenReturn(constructMentorTagAttachResponseDto());

        mockMvc.perform(MockMvcRequestBuilders.post("/mentor-tag/attach")
                                              .contentType(MediaType.APPLICATION_JSON)
                                              .content(objectMapper.writeValueAsString(request)))
               .andExpect(status().isOk())
    ...
    void detachMentorTags_success() {
        ...
        Mockito.when(mentorTagService.detachMentorTag(Mockito.any(MentorTagDetachRequestDto.class)))
               .thenReturn(constructMentorTagDetachResponseDto());

        mockMvc.perform(MockMvcRequestBuilders.post("/mentor-tag/detach")
                                              .contentType(MediaType.APPLICATION_JSON)
                                              .content(objectMapper.writeValueAsString(request)))
               .andExpect(status().isOk())
